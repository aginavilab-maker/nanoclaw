...
export class GmailChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    // Skip polling if tool-only mode
    const envVars = readEnvFile(['GMAIL_POLLING']);
    if (envVars.GMAIL_POLLING === 'false') {
      logger.info(
        'Gmail polling disabled (GMAIL_POLLING=false). Tools-only mode.',
      );
      return;
    }
