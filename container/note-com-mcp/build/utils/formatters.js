// 記事データのフォーマット
export function formatNote(note, username, includeUserDetails, analyzeContent) {
    ...
    const eyecatchUrl = note.eyecatch ||
        note.sp_eyecatch ||
        note.eyecatch_url ||
        note.eyecatchUrl ||
        note.thumbnail ||
    ...
    const publishedAt = note.publishAt ||
        note.publish_at ||
        note.published_at ||
        note.createdAt ||
        note.created_at ||
...
// いいねデータのフォーマット
export function formatLike(like) {
    return {
        id: like.id || "",
...
// マガジンデータのフォーマット
export function formatMagazine(magazine) {
    return {
        id: magazine.id || "",
        name: magazine.name || "",
...
// メンバーシップ記事のフォーマット
export function formatMembershipNote(note) {
    return {
        id: note.id || "",
...
// メンバーシップサマリーのフォーマット
export function formatMembershipSummary(summary) {
...
// 分析用データの集計
export function analyzeNotes(formattedNotes, query, sort) {
    return {
        ...
        engagementAnalysis: {
            ...
            averageComments: formattedNotes.reduce((sum, note) => sum + (note.commentsCount || 0), 0) /
        ...
        contentTypeAnalysis: {
            ...
            averageImageCount: formattedNotes.reduce((sum, note) => sum + (note.contentAnalysis?.imageCount || 0), 0) /
                formattedNotes.length || 0,
        },
        // 価格分析
        priceAnalysis: {
        ...
        authorAnalysis: {
            ...
            averageFollowers: formattedNotes.reduce((sum, note) => sum + (note.author?.details?.followerCount || 0), 0) /
                formattedNotes.length || 0,
