...
let activeGqlAuthToken = null;
export function getActiveSessionCookie() {
    return activeSessionCookie;
}
export function getActiveXsrfToken() {
    return activeXsrfToken;
}
export function getActiveUserKey() {
    return activeUserKey;
}
export function setActiveUserKey(key) {
    activeUserKey = key;
}
export function setActiveSessionCookie(cookie) {
    activeSessionCookie = cookie;
}
export function setActiveXsrfToken(token) {
    ...
    try {
        ...
        try {
            ...
            if (responseData?.data?.key) {
                ...
                if (env.DEBUG)
                    console.error("User key set:", responseData.data.key);
            }
        ...
        if (responseXsrfToken) {
            ...
            if (env.DEBUG)
                console.error("XSRF Token from header:", activeXsrfToken);
        }
        ...
        if (activeSessionCookie && !activeXsrfToken) {
            ...
            try {
                ...
                if (xsrfToken) {
                    ...
                    if (env.DEBUG)
                        console.error("XSRF Token:", activeXsrfToken);
                }
                else {
                    // Set-Cookieヘッダーからも確認
                    ...
                    if (currentUserSetCookieHeader) {
                        ...
                        cookies.forEach((cookieStr) => {
                            if (cookieStr.includes("XSRF-TOKEN=")) {
                                ...
                                if (env.DEBUG)
                                    console.error("XSRF Token from cookie:", activeXsrfToken);
                            }
                        });
                    }
...
export function buildAuthHeaders() {
    ...
    if (activeSessionCookie) {
        ...
        if (env.DEBUG)
            console.error("Using dynamically obtained session cookie for Cookie header");
        if (cookies.length > 0) {
        ...
        if (env.DEBUG)
            console.error("Using session cookie from .env file for Cookie header");
        if (cookies.length > 0) {
            headers["Cookie"] = cookies.join("; ");
        }
    }
    ...
    if (activeXsrfToken) {
        ...
        if (env.DEBUG)
            console.error("Using dynamically obtained XSRF token for X-XSRF-TOKEN header");
    }
    else if (env.NOTE_XSRF_TOKEN) {
        headers["X-XSRF-TOKEN"] = env.NOTE_XSRF_TOKEN;
        if (env.DEBUG)
            console.error("Using XSRF token from .env file for X-XSRF-TOKEN header");
    }
    // User-Agentは常に設定
    headers["User-Agent"] =
...
export async function getPreviewAccessToken(noteId) {
    if (!activeSessionCookie || !activeXsrfToken) {
    ...
    try {
        ...
        if (token) {
            ...
            if (env.DEBUG)
                console.error("Preview Access Token:", token);
            return token;
        }
        else {
