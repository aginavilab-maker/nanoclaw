export class NotionToNoteFormatter {
     ...
     * 箇条書きリストを変換
     */
    bulletListToMarkdown(node, depth = 0) {
     ...
     * 番号付きリストを変換
     */
    numberedListToMarkdown(node, depth = 0) {
        const items = node.children || [];
        const lines = [];
        let counter = 1;
        for (const item of items) {
     ...
     * TODOリストを変換
     */
    todoListToMarkdown(node, depth = 0) {
        const items = node.children || [];
        const lines = [];
        for (const item of items) {
            const text = this.richTextToMarkdown(item.richText || []);
