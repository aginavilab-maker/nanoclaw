...
import { NotionErrorCode, } from "../types/notion-types.js";
...
export class NotionClient {
     ...
     * 指数バックオフ付きリトライ処理
     */
    async retryWithBackoff(operation, retries = RATE_LIMIT_CONFIG.MAX_RETRIES) {
        let lastError;
        for (let i = 0; i <= retries; i++) {
            try {
                ...
                // 指数バックオフで待機
                const delay = Math.min(RATE_LIMIT_CONFIG.INITIAL_RETRY_DELAY * Math.pow(2, i), RATE_LIMIT_CONFIG.MAX_RETRY_DELAY);
    ...
    async getPage(pageId) {
        try {
            const page = await this.retryWithBackoff(() => this.client.pages.retrieve({ page_id: pageId }));
     ...
     * ブロックを再帰的に取得
     */
    async getBlocks(blockId, recursive = true) {
        try {
            const blocks = [];
            let cursor;
            do {
                const response = await this.retryWithBackoff(() => this.client.blocks.children.list({
                    block_id: blockId,
    ...
    async queryDatabase(params) {
        try {
            ...
            const response = await this.retryWithBackoff(() => this.client.databases.query({
