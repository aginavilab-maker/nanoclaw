 ...
 * 画像はプレースホルダーに置換される
 */
function convertObsidianToNoteHtml(markdown, imageBasePath) {
    const images = [];
    let html = markdown;
 ...
 * Obsidian連携ツールを登録する
 */
export function registerObsidianTools(server) {
    /**
     * Obsidian MarkdownをnoteのHTML形式に変換
     */
    server.tool("convert-obsidian-markdown", "Obsidian MarkdownをnoteのHTML形式に変換（画像はプレースホルダーに置換）", {
     ...
     */
    server.tool("prepare-obsidian-draft", "Obsidian記事をnote下書き用に準備（画像情報を含む）", {
