...
import { parseMarkdown, formatToNoteEditor, extractTitle, removeTitle, removeFrontmatter, } from "../utils/note-editor-formatter.js";
/**
 * 構造的な行（見出し、リスト、コードブロック、画像等）かどうかを判定
 */
function isImageOrStructuralLine(line) {
 ...
 * 現在のカーソル位置に画像を挿入
 */
async function insertImageAtCurrentPosition(page, bodyBox, imagePath) {
    // 新しいパラグラフを作成
...
async function setEyecatchImage(page, imagePath) {
    const selectors = [
    ...
    const uploadMenuSelector = '[role="menuitem"]:has-text("画像をアップロード"), [role="option"]:has-text("画像をアップロード"), button:has-text("画像をアップロード"), div:has-text("画像をアップロード"):not(:has(*:has-text("画像をアップロード")))';
    const fallbackMenuSelector = '[role="menuitem"]:has-text("画像"), [role="option"]:has-text("画像"), button:has-text("画像"), div:has-text("画像"):not(:has(*:has-text("画像")))';
    const openMenuAndGetFileChooser = async () => {
    ...
    if (!chooser) {
        ...
        if (bodyBoxHandle) {
            ...
            for (const el of candidates) {
                ...
                if (!box)
                    continue;
                if (box.y >= bodyBoxHandle.y)
                    continue;
                if (box.y < Math.max(bodyBoxHandle.y - 500, 0))
                    continue;
                if (box.x < bodyBoxHandle.x || box.x > bodyBoxHandle.x + bodyBoxHandle.width)
                    continue;
...
async function waitForFirstVisibleLocator(page, selectors, timeoutMs) {
    ...
    throw new Error(`タイトル入力欄が見つかりませんでした: ${selectors.join(", ")}\n${lastError?.message || ""}`);
}
async function fillNoteTitle(page, title) {
 ...
 */
async function createNoteWithPlaywright(title, markdown, imageBasePath, options = {}) {
    ...
    try {
        ...
        let eyecatchImagePath = null;
        let eyecatchCaption = null;
        const eyecatchIndex = elements.findIndex((element) => element.type === "image" && Boolean(element.imagePath));
        if (eyecatchIndex !== -1) {
...
export function registerPublishTools(server) {
     ...
     */
    server.tool("publish-from-obsidian", "Obsidian記事をnoteに公開（エディタUI操作で書式を適用、画像を自動挿入）", {
     ...
     * 本文に画像を挿入（既存の下書きに対して）
     */
    server.tool("insert-images-to-note", "noteエディタで本文に画像を挿入（Playwright使用）", {
        ...
        try {
            ...
            await bodyBox.click();
            const keyCombos = process.platform === "darwin" ? ["Meta+ArrowDown", "End"] : ["Control+End", "End"];
     ...
     */
    server.tool("publish-from-obsidian-remote", "Obsidian記事をnoteに公開（画像データをBase64で受信、リモートサーバー用）", {
        ...
        try {
            ...
            // Playwrightで記事を作成
            const result = await createNoteWithPlaywright(title, processedMarkdown, tempDir, // 一時ディレクトリを画像ベースパスとして使用
