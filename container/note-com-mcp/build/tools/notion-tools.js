...
import { NoteImageUploader } from "../utils/note-image-uploader.js";
import { DEFAULT_PAGE_SIZE } from "../config/notion-config.js";
import { createSuccessResponse, createErrorResponse, handleApiError, } from "../utils/error-handler.js";
...
export function registerNotionTools(server) {
    ...
    server.tool("list-notion-pages", "List pages from a Notion database", {
    ...
    server.tool("get-notion-page", "Get detailed information about a Notion page", {
    ...
    server.tool("preview-notion-to-note", "Preview how a Notion page will be converted to note.com format", {
    ...
    server.tool("import-notion-to-note", "Import a Notion page to note.com as a draft", {
        ...
        try {
            const result = await importNotionToNote(notionClient, blockParser, formatter, pageId, tags || [], saveAsDraft !== false);
            if (result.success) {
                return createSuccessResponse({
 ...
 */
async function importNotionToNote(notionClient, blockParser, formatter, pageId, tags, saveAsDraft) {
    const result = {
        success: false,
        stats: {
            total_blocks: 0,
    ...
    try {
        ...
        for (const ref of imageReferences) {
            try {
                // IRから画像URLを取得
                const imageNode = irNodes.find((node) => node.type === "image" && node.content && ref.includes(node.content.split("?")[0]));
