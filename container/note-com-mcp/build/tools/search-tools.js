...
import { createSuccessResponse, createErrorResponse, handleApiError, safeExtractData, safeExtractTotal, commonExtractors, } from "../utils/error-handler.js";
...
export function registerSearchTools(server) {
    ...
    server.tool("analyze-notes", "記事の詳細分析を行う（競合分析やコンテンツ成果の比較等）", {
        ...
        try {
            ...
            const data = await noteApiRequest(`/v3/searches?context=note&${params.toString()}`, "GET", null, true);
            if (env.DEBUG) {
            ...
            const notesArray = safeExtractData(data, commonExtractors.notes);
            const totalCount = safeExtractTotal(data, notesArray.length);
            const formattedNotes = notesArray.map((note) => formatNote(note, undefined, includeUserDetails, analyzeContent));
    ...
    server.tool("search-users", "ユーザーを検索する", {
        ...
        try {
            ...
            const formattedUsers = usersArray.map((user) => formatUser(user));
            return createSuccessResponse({
                total: totalCount,
    ...
    server.tool("search-magazines", "マガジンを検索する", {
        ...
        try {
            ...
            const formattedMagazines = magazinesArray.map((magazine) => formatMagazine(magazine));
            return createSuccessResponse({
                total: totalCount,
    ...
    server.tool("search-all", "note全体検索（ユーザー、ハッシュタグ、記事など）", {
        ...
        try {
            const data = await noteApiRequest(`/v3/searches?context=${encodeURIComponent(context)}&mode=${encodeURIComponent(mode)}&q=${encodeURIComponent(query)}&size=${size}&sort=${sort}`, "GET", null, false);
            const result = {
                query,
                context,
                mode,
